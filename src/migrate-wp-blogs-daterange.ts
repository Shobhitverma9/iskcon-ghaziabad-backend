...
// ── Date range (Jan 8, 2026 → Feb 5, 2026) ────────────────────────────────────
const AFTER  = '2026-01-07T18:30:00'; // UTC equivalent of IST Jan 8 midnight
const BEFORE = '2026-02-05T18:30:00'; // UTC equivalent of IST Feb 6 midnight

const WP_API_URL = `https://iskconghaziabad.co.in/wp-json/wp/v2/posts?per_page=100&_embed&after=${AFTER}&before=${BEFORE}`;
...
async function bootstrap() {
    ...
    try {
        ...
        console.log(`\nImporting WP posts with Cloudinary Uploads: Jan 8, 2026 → Feb 5, 2026\n`);
            ...
            for (const post of wpPosts) {
                ...
                try {
                    ...
                    if (existing) {
                        ...
                        totalMigrated++;
                        console.log('     ♻️  Replaced with Cloudinary images.');
                    } else {
                        await blogService.create(payload);
                        totalMigrated++;
                        console.log('     ✅ Created with Cloudinary images.');
                    }
                } catch (err: any) {
        ...
        console.log(`\n✅ Migration (with Cloudinary) complete. Total imported: ${totalMigrated}\n`);
