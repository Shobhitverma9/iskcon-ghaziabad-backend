...
async function sendEkadashiCampaign() {
  ...
  // Note: Replace the image URL with the actual hosted one after uploading
  const htmlBody = `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 12px; overflow: hidden; background-color: #fff;">
        ...
        <div style="background-color: #f9f9f9; padding: 25px; text-align: center; color: #888; font-size: 12px; border-top: 1px solid #eee;">
            ...
            <p style="margin: 5px 0 0;">You are receiving this because of your pious connection with ISKCON Ghaziabad.</p>
            <p style="margin: 10px 0 0;"><a href="https://iskconghaziabad.com/unsubscribe?email={{Email}}" style="color: #888; text-decoration: underline;">Unsubscribe from this list</a></p>
        </div>
    </div>
  `;

  console.log(`Sending Kamada Ekadashi campaign to ${testEmails.length} recipients...`);
  try {
    await notificationService.sendBroadcastBatch(testEmails, subject, htmlBody);
    console.log('✅ Campaign test submitted successfully!');
  } catch (error) {
